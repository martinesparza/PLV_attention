function stdshade(amatrix,alpha,style,acolor,shade,F,smth)
...
amean = abs(nanmean(amatrix,1)); %get man over first dimension
if smth > 1
...
if exist('alpha','var')==0 || isempty(alpha) 
    ...
    acolor='k';
else fill([F fliplr(F)],[amean+astd fliplr(amean-astd)],shade, 'FaceAlpha', alpha,'linestyle','none');    
end
if ishold==0
    check=true; else check=false;
end
hold on;plot(F,amean,'LineStyle',style,'Color',acolor,'linewidth',2.5); %% change color or linewidth to adjust mean line
if check
    hold off;
end
end
